...
export const load: PageLoad = async ({ params }) => {
	...
	const trackPaths: Record<string, { metadata: Track; default: Component }> = import.meta.glob(
		`/src/tracks/*.md`,
		{ eager: true }
	);
	...
	const imgKey = Object.keys(imageModules).find((key) => key.includes(params.slug));
